...
describe("extract-profile tests - ", () => {
  ...
  it("Email", () => {
    expect(matchEmail(makeTextItem("  hello@airesume.org  "))![0]).toBe(
      "hello@airesume.org"
    );
  });
  ...
  it("Url", () => {
    expect(matchUrl(makeTextItem("  linkedin.com/in/airesume  "))![0]).toBe(
      "linkedin.com/in/airesume"
    );
    expect(matchUrl(makeTextItem("hello@airesume.org"))).toBeFalsy();
  });
});
